...
class PinWizard {
	constructor(config) {
		if (!config || !config.ipfs || !config.ipfs.host || !config.ipfs.port)
			throw new Error("IPFS Config Details are required! Please make sure you are setting the config.ipfs variable, the config.ipfs.host, and the config.ipfs.port settings.")

		this.config_ = config

		this.ipfs_api = ipfsAPI(this.config_.ipfs)

		let index_config

		if (this.config_.OIPdURL)
			index_config = {OIPdURL: this.config_.OIPdURL}

		this.index_ = new Index(index_config)
	}

	async getAllIPFSLocations(){
		let artifacts 
		try {
			artifacts = await this.index_.getArtifacts(this.config_.type, this.config_.subtype, undefined, "*")
		} catch (e) {
	...
	async updateClusterPins() {
		...
		var totalPinned = locations.length - unpinned_locations.length, newPins = 0, unableToPin = 0;

		let pin_promises = []

		for (var location of unpinned_locations){
			var prom = this.ipfs_api.pin.add(location)
